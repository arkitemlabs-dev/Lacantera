...
import { Eye, FileDown, ListFilter, Calendar as CalendarIcon } from 'lucide-react';
import { purchaseOrders } from '@/lib/data';
import type { PurchaseOrderStatus, PurchaseOrder } from '@/lib/types';
...
export default function OrdenesDeCompraPage() {
    ...
    const filteredOrders = useMemo(() => {
      return purchaseOrders.filter((order) => {
        ...
        const orderNumberFilter = order.id.toLowerCase().includes(orderNumber.toLowerCase()) || order.name.toLowerCase().includes(orderNumber.toLowerCase());
          ...
          <TableBody>
            {filteredOrders.map((order) => (
              <TableRow key={order.id}>
                ...
                <TableCell>
                  <Badge
                    variant={statusStyles[order.status as keyof typeof statusStyles].variant}
                    className={cn('font-normal', statusStyles[order.status as keyof typeof statusStyles].className)}
                  >
                    {order.status}
                  </Badge>
                </TableCell>
